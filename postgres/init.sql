...
-- 2. 사용자 정보를 저장하는 메인 유저 테이블 생성 (백엔드 코드 요구사항 반영 및 최종 유지)
CREATE TABLE IF NOT EXISTS users (
    ...
    nickname VARCHAR(100) NOT NULL UNIQUE,                  -- 사용자 닉네임 (상대방 모델 반영 unique 추가)
    profile_image VARCHAR(512) NULL,                       -- 프로필 이미지 URL 경로
    chat_limit_override INT NULL,                          -- Nullable / 유저별 일일 AI 챗봇 이용 제한량
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP, -- 타임존 일치
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP, -- 타임존 일치
    email VARCHAR(255) NOT NULL UNIQUE                     -- Email
);

-- 3. 기출문제 및 문제은행 저장을 위한 메인 테이블 생성 (확장 명세 반영 유지)
