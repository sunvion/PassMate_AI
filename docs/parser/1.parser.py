    ...
    def reconstruct_lines(col_words):
        ...
        for line_w in lines_words:
            ...
            formatted_lines.append({
                ...
                "x0": min(w["x0"] for w in line_w),
                "x1": max(w["x1"] for w in line_w),

                "top": min(w["top"] for w in line_w),
                "bottom": max(w["bottom"] for w in line_w)
            })
            ...
            for column_name, col_lines in [
                ...
                for line_obj in col_lines:
                    ...
                    if any(meta in text_val for meta in [
                        ...
                        continue
                    line_obj["page_idx"] = idx
                    line_obj["column"] = column_name
